...
def _get_bitmap_address(metadata, page_number):
    ...
    """
    addresses = []
    key_layers_retrieved = metadata.pages[page_number][fileformat.KEY_LAYERS]  # PySN
    if len(key_layers_retrieved) == 0:  # PySN
        return []  # PySN
...
class SupernoteParser:
    ...
    def parse_stream(self, stream, policy='strict'):
        ...
        if 'HIGHLIGHTINFO' in header.keys():  # PySN
            highlight_info = _get_content_at_address(stream, int(header['HIGHLIGHTINFO']))  # PySN
            if highlight_info is not None:  # PySN
                highlight_info_dict = json.loads(base64.b64decode(highlight_info).decode('utf-8'))  # PySN
                header["HIGHLIGHTDETAILS"] = highlight_info_dict  # PySN
...
class SupernoteXParser(SupernoteParser):
    ...
    def _parse_link_block(self, fobj, address):
        address_dict = self._parse_metadata_block(fobj, address)  # PySN
        web_like_link = address_dict['LINKTYPE'] == "4"  # PySN
        if web_like_link:  # PySN
            encoded_url = address_dict['LINKFILE']  # PySN
            url = base64.b64decode(encoded_url).decode()  # PySN
            address_dict['LINKURL'] = url  # PySN
        return address_dict  # PySN
        # return self._parse_metadata_block(fobj, address)   # PySN
