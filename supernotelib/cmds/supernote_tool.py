...
def convert_to_png(args, notebook, palette):
    if args.mark:  # PYSN
        converter = ImageConverter(notebook, palette=palette, mark=True)  # PYSN
    else:  # PYSN
        converter = ImageConverter(notebook, palette=palette)  # PYSN
    # converter = ImageConverter(notebook, palette=palette)  # PYSN
...
def main():
    ...
    parser_convert.add_argument('--mark', action='store_true', default=False, help='for mark files')  # PYSN
